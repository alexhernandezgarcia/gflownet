#!/bin/bash
# =============================================================================
# ONE composite-tree regression run: train (or resume), then evaluate.
# =============================================================================
#
...
#
# Usage:
#   reg_tree_worker.sh <split> [extra hydra overrides...]
#
...
#   EXP_NAME      campaign name / top run-dir level       (default TREECLASS_REG)
#   EXP_CONFIG    config under config/experiments         (default tree/regression_tree)
#   DATASET       dataset dir under tests/data/tree       (default diabetes)
#   SEED          random seed                             (default 0)
#   RUNS_ROOT     root of the run tree                    (default $SCRATCH/gflownet-logs)
#   TOP_K         trees ranked for top-k metrics          (default 10)
...
split="${1:?usage: reg_tree_worker.sh <split> [hydra overrides...]}"
shift
...
# -----------------------------------------------------------------------------
work_tmp="${SLURM_TMPDIR:-/tmp/$USER}/reg_tree_${DATASET}_${split}_${SEED}_$$"
mkdir -p "$work_tmp"
