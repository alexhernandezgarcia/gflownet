...
def main(args):
    ...
    for smile in args.ids:
        ...
        env = Conformer(
            smiles=int(smile), n_torsion_angles=-1, reward_func=REWARD_FUNC, reward_beta=REWARD_BETA
        )
        ...
        if ndims == 6:
            def reward(p0, p1, p2, p3, p4, p5):
                batch = np.concatenate([[p0], [p1], [p2], [p3], [p4], [p5]]).reshape(1, -1)
                batch = np.concatenate([batch, np.zeros((batch.shape[0], 1))], axis=-1)
            ...
            def reward(p0, p1, p2, p3, p4, p5 ,p6):
                batch = np.concatenate([[p0], [p1], [p2], [p2], [p3], [p4], [p5], [p6]]).reshape(1, -1)
                batch = np.concatenate([batch, np.zeros((batch.shape[0], 1))], axis=-1)
            ...
            def reward(p0, p1, p2, p3, p4, p5 ,p6, p7):
                batch = np.concatenate([[p0], [p1], [p2], [p3], [p4], [p5], [p6], [p7]]).reshape(1, -1)
                batch = np.concatenate([batch, np.zeros((batch.shape[0], 1))], axis=-1)
            ...
            def reward(p0, p1, p2, p3, p4, p5 ,p6, p7, p8):
                batch = np.concatenate([[p0], [p1], [p2], [p3], [p4], [p5], [p6], [p7], [p8]]).reshape(1, -1)
                batch = np.concatenate([batch, np.zeros((batch.shape[0], 1))], axis=-1)
            ...
            def reward(p0, p1, p2, p3, p4, p5 ,p6, p7, p8, p9):
                batch = np.concatenate([[p0], [p1], [p2], [p3], [p4], [p5], [p6], [p7], [p8], [p9]]).reshape(1, -1)
                batch = np.concatenate([batch, np.zeros((batch.shape[0], 1))], axis=-1)
            ...
            def reward(p0, p1, p2, p3, p4, p5 ,p6, p7, p8, p9, p10):
                batch = np.concatenate([[p0], [p1], [p2], [p3], [p4], [p5], [p6], [p7], [p8], [p9], [p10]]).reshape(1, -1)
                batch = np.concatenate([batch, np.zeros((batch.shape[0], 1))], axis=-1)
            ...
            def reward(p0, p1, p2, p3, p4, p5 ,p6, p7, p8, p9, p10, p11):
                batch = np.concatenate([[p0], [p1], [p2], [p3], [p4], [p5], [p6], [p7], [p8], [p9], [p10], [p11]]).reshape(1, -1)
                batch = np.concatenate([batch, np.zeros((batch.shape[0], 1))], axis=-1)
        ...
        if gdsamples.samples.shape[0] >= 1000:
            ...
            dct = {"x": gdsamples.samples[-1000:, :npars]} #, "energy": np.exp(gdsamples.loglikes[-10000:])}

            dct["energy"] = get_energy(gdsamples.samples[-1000:, :npars])

            dct["conformer"] = [env.set_conformer(state).rdk_mol for state in gdsamples.samples[-1000:, :npars]]
            pickle.dump(
                dct, open(output_dir / f"conformers_mcmc_smile{smile}_{args.proxy_name}.pkl", "wb")
            )
