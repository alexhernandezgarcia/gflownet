...
from gflownet.utils.molecule.metrics import get_cov_mat, get_best_rmsd
    ...
    for conf in confs_list[1:]:
        if distant_enough(conf, result, delta):
            ...
            if len(result) >= k:
                break
    if len(result) < k:
        print(f"Cannot find {k} different conformations in {len(confs_list)} for delta {delta}")
...
def main(args):
    ...
    if args.use_top_k:
        ...
        if args.diverse:
            gen_confs = [get_diverse_top_k(x, k=len(ref)*2, delta=args.delta) for x, ref in zip(gen_confs, ref_confs)]
    if not args.hack:
    ...
    for ref, gen, sm in tqdm(zip(ref_confs, gen_confs, smiles), total=len(ref_confs)):
        if len(gen) < 2*len(ref):
            ...
            print(f"Computing metrics with {len(gen)} generated confs for {len(ref)} reference confs")
        try:
            if len(gen) > 2*len(ref):
                hack = True
                print(f"Warning! Computing metrics with {len(gen)} generated confs for {len(ref)} reference confs") 
