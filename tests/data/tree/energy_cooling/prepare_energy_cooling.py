"""
Generate energy_cooling_1.csv through energy_cooling_5.csv in DT-GFN CSV format
from the UCI Energy Efficiency dataset (768 samples, 8 features; regression
target: cooling load Y2; the first target Y1, heating load, is dropped).

Companion to tests/data/tree/energy/prepare_energy.py, which uses Y1 (heating
load) as the target. Reuses the ENB2012_data.xlsx already downloaded there if
present; otherwise downloads it from the UCI repository (requires openpyxl to
read the xlsx).

Run from: tests/data/tree/energy_cooling/
    python prepare_energy_cooling.py
"""
...
XLSX_PATHS = [Path(XLSX_NAME), Path("..") / "energy" / XLSX_NAME]
...
for seed in [1, 2, 3, 4, 5]:
    ...
    pd.concat([df_train, df_test], ignore_index=True).to_csv(
        f"energy_cooling_{seed}.csv", index=False
    )
