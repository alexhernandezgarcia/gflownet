"""
Generate qsar_fish_1.csv through qsar_fish_5.csv in DT-GFN CSV format from
the UCI QSAR Fish Toxicity dataset (908 samples, 6 molecular descriptors;
regression target: LC50 towards Pimephales promelas / fathead minnow, in
-log10(mol/L)).

Downloads qsar_fish_toxicity.csv from the UCI repository on first run.

Run from: tests/data/tree/qsar_fish/
    python prepare_qsar_fish.py
"""
...
UCI_URL = "https://archive.ics.uci.edu/static/public/504/qsar+fish+toxicity.zip"
DATA_NAME = "qsar_fish_toxicity.csv"
...
print(f"Shape: {X.shape} | target range: [{y.min()}, {y.max()}]")  # (908, 6)
...
for seed in [1, 2, 3, 4, 5]:
    ...
    pd.concat([df_train, df_test], ignore_index=True).to_csv(
        f"qsar_fish_{seed}.csv", index=False
    )
