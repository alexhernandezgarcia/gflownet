...
def test__get_action_space__returns_expected(env, action_space):
    assert len(env.action_space) == len(env.sectors) + len(env.tags) + len(env.techs) + len(env.amounts) + 1
...
def test__get_parents__returns_expected(
    ...
    expected_pairs = set(
        (tuple(sorted(p.items())), a) for p, a in zip(parents_expected, parents_a_expected)
    )
