...
@pytest.fixture
def proxy_no_norm():
    return CompositionMPFrequency(normalise=False, device=DEVICE, 
            float_precision=FLOAT)

@pytest.fixture
def proxy_norm():
    return CompositionMPFrequency(normalise=True, device=DEVICE, 
            float_precision=FLOAT)
...
def test_max_protons_number(proxy_no_norm, elements, required_elements, max_diff_elem, 
        min_diff_elem, min_atoms,max_atoms, min_atom_i, max_atom_i, expected_state):
    env = Composition(proxy=proxy_no_norm,
            elements = elements,
...
def test_proxy_call(proxy_no_norm, small_env_no_norm, state):
    with open(Path(__file__).parents[4] 
            /'gflownet/proxy/crystals/number_of_protons_counts.pkl', 'rb') as handle:
    ...
    assert expected == proxy_no_norm(
            small_env_no_norm.statetorch2proxy(
                torch.tensor(
                    [state]
                    ).to(proxy_no_norm.atomic_numbers)
                )
            ).item()
