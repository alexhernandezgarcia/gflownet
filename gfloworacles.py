...
def linearToy(queries, epsilon=1e-9):
    ...
    :return:
    '''
    queries, ndim = preprocess_queries(queries)
    linFactors = np.ones(queries.shape[1]) # even simpler
    ...
    # Turn energies into rewards > 0 for GFlowNet
    energies += epsilon
...
def toyHamiltonian(queries, epsilon=1e-9):
    '''
    return the energy of a toy model for the given set of queries
...
def PottsEnergy(queries, nalphabet=4, epsilon=1e-9):
    '''
...
def seqfoldScore(queries, returnSS = False, epsilon=1e-9):
    '''
...
'''
def nupackScore(queries, returnSS=False, parallel=True, epsilon=1e-9):
