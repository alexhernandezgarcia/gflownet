...
# (tests/data/tree/iris/iris_<split>.csv), so train/test splits are identical.
#
# Run names use "depth5" (the legacy-equivalent label) so the wandb runs line
# up side by side with LEGACYCODE_NOLEAK_iris<split>_depth5_steps100.
#
# Usage:
#   mkdir -p $SCRATCH/gflownet-logs/slurm && sbatch mila/sbatch/compare_tree_legacy_iris.sh
# Rerun a single split, e.g. split 3:
#   sbatch --array=3 mila/sbatch/compare_tree_legacy_iris.sh
#
