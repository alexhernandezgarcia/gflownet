...
# =============================================================================
# GPU variant of run_regression_tree_training.sh.
# =============================================================================
#
# Differences to the CPU script (mirrors run_classification_tree_training_gpu.sh):
...
#   * EXP_CONFIG defaults to tree/trfm_regression_tree: the transformer policy
#     is the one worth putting on a GPU (and the one to profile there); pass
#     EXP_CONFIG=tree/regression_tree for the MLP.
...
# backward one, so activation memory scales with the total number of states in
# the batch -- which grows during training as the sampler learns to build
# bigger trees. Measured on diabetes with this config (max_depth=5, d_model=128,
# n_layers=3, 100 trajectories/batch, smoke tests 2026-08-20): ~1.7 MiB per
# state, plateauing around ~13.5k states => ~23 GB typical peak, with spikes
# that OOM a 32 GB V100 (observed). Hence --gres pins an l40s (48 GB). For a
# 32 GB card, halve gflownet.optimizer.batch_size instead. Deeper trees or a
# larger d_model scale this up accordingly (a100l / h100 territory).
#
# Everything else (run layout, LAUNCH records, resume, evaluation) is
# identical -- see run_regression_tree_training.sh for the full docs.
#
# Usage:
#   mkdir -p $SCRATCH/gflownet-logs/slurm
#   sbatch --export=ALL,EXP_NAME=TRFM_REG_GPU \
#          mila/tree/run_regression_tree_training_gpu.sh
#
#   # single split, extra overrides (e.g. for profiling):
#   sbatch --array=1 --export=ALL,EXP_NAME=TRFM_REG_GPU_PROF \
#          mila/tree/run_regression_tree_training_gpu.sh \
#          gflownet.optimizer.n_train_steps=500 profile=true
#
# Environment knobs (all overridable via --export=ALL,VAR=value):
#   EXP_NAME    campaign name                       (default: TREECLASS_REG_GPU)
#   EXP_CONFIG  experiment config under config/experiments
#                                           (default: tree/trfm_regression_tree)
...
EXP_NAME="${EXP_NAME:-TREECLASS_REG_GPU}"
EXP_CONFIG="${EXP_CONFIG:-tree/trfm_regression_tree}"
