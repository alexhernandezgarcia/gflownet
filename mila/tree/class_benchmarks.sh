#!/bin/bash
#SBATCH --job-name=class_benchmarks
#SBATCH --output=/home/mila/a/arnit/gflownet/class_benchmarks/slurm/%x-%A_%a.out
...
#
# All outputs live in $REPO/class_benchmarks/: SLURM logs in slurm/, result
# JSONs in results/ (the run_*.py default). Summarize any time with:
#   python class_benchmarks/print_results.py
#
# Usage (from anywhere; the script submits itself):
#   bash mila/tree/class_benchmarks.sh
#
...
# class_benchmarks.sh) only those methods are (re)run -- MAPTree and the
...
SCRIPT="$REPO/mila/tree/class_benchmarks.sh"
...
if [ -z "${SLURM_ARRAY_TASK_ID:-}" ]; then
    mkdir -p "$REPO/class_benchmarks/slurm"
...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ]; then
    python class_benchmarks/run_cart.py --datasets magic \
        --max-depth "$MAX_DEPTH" || status=$?
    python class_benchmarks/run_gbt.py --datasets magic \
        --max-depth "$MAX_DEPTH" || status=$?
    if [ "$MAX_DEPTH" -eq 5 ]; then
        python class_benchmarks/run_maptree.py --datasets magic \
            --thresholds "$BINARIZATION_THRESHOLDS" \
            --time-limit "$MAPTREE_TIME_LIMIT" || status=$?
        python class_benchmarks/run_bcart.py --datasets magic \
