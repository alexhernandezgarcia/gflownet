...
class fairy_model(nn.Module):
    def __init__(self, variables_dim, variables_names, subsidies_dim, subsidies_names, num_blocks, dim_block, width_block, activation, normalization='batch', dropouts=0.0, probabilistic = True,
                 scaling = 'original'):
        ...
        if scaling not in ['original', 'normalization', 'maxscale']:
            raise ValueError('Scaling type must be either original, normalization, or maxscale\n Unknown scaling type: {}'.format(scaling))

        blocks = []

        blocks.append(nn.Linear(self.variables_dim + self.subsidies_dim, self.width_block))
        for _ in range(self.num_blocks):
            blocks.append(
                residual_block(self.width_block, self.dim_block, self.activation, self.normalization, self.dropouts))
...
def initialize_fairy() -> Tuple[fairy_model, witch_proc_data]:
    ...
    dataset_filename = 'gflownet/proxy/iam/scenario_data/witch_scenario_maxscale_dataset.pkl'
    ...
    fairy = fairy_model(variables_dim=variables_dim,
