...
class Conformer(ConformerBase):
    ...
    def set_atom_positions_dgl(self, atom_positions):
        ...
        self._dgl_graph.ndata[constants.atom_position_name] = torch.Tensor(atom_positions)
...
if __name__ == '__main__':
    ...
    conf = Conformer(test_pos, constants.ad_smiles, 
    ...
    actions = np.random.uniform(-1, 1, size=len(conf._dgl_graph.edges()[0]) // 2)*np.pi
    ...
    actions_dict = {ta: actions[conf.get_ta_index_in_dgl_graph(ta)//2] for ta in conf.freely_rotatable_tas}
    data = [[k, a, (conf_tas[k] + a), new_conf_tas[k]] for k, a in actions_dict.items()]
    print(tabulate(data, headers=["torsion angle", 'action', 'init + action', 'after action']))

    # check dgl_graph
    conf.randomize_freely_rotatable_tas()
    print('rdk pos', conf.get_atom_positions()[3])
    print('_dgl pos (should differ from rdk)', conf._dgl_graph.ndata[constants.atom_position_name][3])
    print('dgl pos (should be the same as rdk)', conf.dgl_graph.ndata[constants.atom_position_name][3])
