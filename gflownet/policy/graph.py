...
class Policy:
    def __init__(self, config, env, device, float_precision, base=None):
        ...
        self.conditioning_dim = config.policy_conditioning_dim
    ...
    def instantiate(self):
        ...
        self.conditioner = self.make_graph_conditioner()

    def __call__(self, states, conditions):
        conditions_encoding = self.conditioner(conditions)
        return self.model(torch.cat((states, conditions_encoding), dim=-1))
    ...
    def make_mlp(self, activation):
        ...
        if self.shared_weights == True and self.base is not None:
            ...
            layers_dim = (
                    [self.state_dim + self.conditioning_dim] + [self.n_hid] * self.n_layers + [(self.output_dim)]
            )
            mlp = nn.Sequential(
                *(
                        sum(
    ...
    def make_graph_conditioner(self):
        from MCryGAN.standalone import graph_conditioner
        # todo add conditioner params to config (there are many)
        print("Building graph conditioner with fixed parameters")
        return graph_conditioner()
