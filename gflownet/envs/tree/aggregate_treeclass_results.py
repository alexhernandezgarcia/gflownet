...
def compute_metrics(pkl_path: Path, meta: dict, n_dirichlet_samples: int,
                    use_cache: bool) -> dict:
    ...
    tree_stats = calculate_tree_accuracies(
        states, log_posteriors, X_train, y_train, X_test, y_test,
        alpha, n_classes, node_env, n_dirichlet_samples,
    )
...
def main():
    ...
    if not selected:
        print(f"No finished runs found under {args.logs_root} "
