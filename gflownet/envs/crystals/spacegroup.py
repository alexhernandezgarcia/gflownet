...
class SpaceGroup(GFlowNetEnv):
    ...
    def get_mask_invalid_actions_forward(
        ...
        if cls_idx != 0:
            crystal_lattice_systems = []
            space_groups_cls = [
                (self.sg_idx, self.sg2stateind[sg], state_type)
            ...
            if ps_idx == 0:
                point_symmetries = [
                    (self.ps_idx, self.pg2stateind[idx], state_type)
        ...
        if ps_idx != 0:
            ...
            if cls_idx == 0:
                crystal_lattice_systems = [
                    (self.cls_idx, self.cls2stateind[idx], state_type)
    ...
    def _is_compatible(  # todo retest this
        ...
        space_groups = [self.n_atoms_compatibility_dict[sg] for sg in np.arange(1,231)]

        # Prune the list of space groups to those compatible with the provided crystal-
        # lattice system
        if cls_idx is not None:
            space_groups_cls = self.crystal_lattice_systems[self.stateind2cls[cls_idx]]["space_groups"]
        ...
        if ps_idx is not None:
            space_groups_ps = self.point_symmetries[self.stateind2pg[ps_idx]]["space_groups"]
