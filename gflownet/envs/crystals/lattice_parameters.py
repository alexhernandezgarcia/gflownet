...
class LatticeParameters(ContinuousCube):
    ...
    def cube2latticeparams(self,cube_states,crystal_lattice_systems):
        '''
        Converts a list of cube-states and their corresponding crystal lattice systems to the lattice parameters format
        (used to be in _step)
        '''
    ...
    def _setup_constraints_read_only(self,lattice_system):
        '''
        This is a version of setup_constraints that does all the same stuff but can be used on the read-only env
        FIXME this is hacky and should be redone in a better way
        '''
