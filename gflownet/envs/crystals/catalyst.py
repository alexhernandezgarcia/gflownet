...
class Catalyst(GFlowNetEnv):
    """
    A combination of Composition, SpaceGroup, CLatticeParameters and MillerIndices into
    a single environment. Works sequentially, by first filling in the Composition, then
    SpaceGroup, CLatticeParameters and finally MillerIndices.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.do_sg_to_miller_constraints = do_sg_to_miller_constraints
        ...
        self.miller_indices_kwargs = miller_indices_kwargs or {}
        ...
        miller_indices = MillerIndices(
            is_hexagonal_rhombohedral=False, **self.miller_indices_kwargs
        )
        self.subenvs = OrderedDict(
            {
                ...
                Stage.MILLER_INDICES: miller_indices,
            }
        )

        # 0-th element of state encodes current stage: 0 for composition,
        # 1 for space group, 2 for lattice parameters, 3 for Miller indices
        ...
        # EOS is EOS of the last stage (Miller indices)
        self.eos = self._pad_action(
            self.subenvs[Stage.MILLER_INDICES].eos, Stage.MILLER_INDICES
        )
    ...
    def reset(self, env_id: Union[int, str] = None):
        ...
        self.subenvs[Stage.MILLER_INDICES] = MillerIndices(
            is_hexagonal_rhombohedral=False, **self.miller_indices_kwargs
        )
    ...
    def _get_previous_stage(self, stage: Stage) -> Stage:
        ...
        Composition -> SpaceGroup -> LatticeParameter -> MillerIndices -> Done
        or
        SpaceGroup -> Composition -> LatticeParameter -> MillerIndices -> Done
        """
        if self.do_sg_before_composition:
            ...
            elif stage is Stage.MILLER_INDICES:
                return Stage.LATTICE_PARAMETERS
            elif stage is Stage.DONE:
                return Stage.MILLER_INDICES
            else:
    ...
    def step(
        ...
        if action_subenv == self.subenvs[stage].eos:
            ...
            if stage is Stage.SPACE_GROUP:
                ...
                if self.do_sg_to_miller_constraints:
                    lattice_system = self.subenvs[Stage.SPACE_GROUP].lattice_system
                    self.subenvs[Stage.MILLER_INDICES].set_lattice_system(
                        lattice_system
                    )
    ...
    def set_state(self, state: List, done: Optional[bool] = False):
        ...
        if stage is Stage.COMPOSITION and self.do_sg_before_composition:
            ...
            done_subenvs[Stage.COMPOSITION] = True
            done_subenvs[Stage.SPACE_GROUP] = True
            done_subenvs[Stage.LATTICE_PARAMETERS] = True
        elif stage is Stage.DONE:
            for subenv in done_subenvs:
                done_subenvs[subenv] = True
        done_subenvs[Stage.MILLER_INDICES] = done
        ...
        if self.subenvs[Stage.SPACE_GROUP].done:
            ...
            (if it was set) or reset it to the default triclinic otherwise. Similarly,
            the MillerIndices's is_hexagonal_rhombohedral is set or reset to the
            default False otherwise. Why this is needed: for backward sampling, where
            we start from an arbitrary terminal state and need to synchronize the
            LatticeParameter's and MillerIndices' lattice system to what that state
            indicates.
            """
            lattice_system = self.subenvs[Stage.SPACE_GROUP].lattice_system
            ...
            if lattice_system != "None" and self.do_sg_to_miller_constraints:
                self.subenvs[Stage.MILLER_INDICES].set_lattice_system(lattice_system)
            else:
                self.subenvs[Stage.MILLER_INDICES].is_hexagonal_rhombohedral = False
