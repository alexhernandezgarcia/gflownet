...
class BaseEvaluator(AbstractEvaluator):
    def __init__(self, gfn_agent=None, **config):
        ...
        if not getattr(self.config, "compute_logz_est", False):
            if isinstance(self.metrics, dict) and "logz_est" in self.metrics:
                self.metrics = {
                    k: v for k, v in self.metrics.items() if k != "logz_est"
                }
                self.reqs = set(
    ...
    def define_new_metrics(self):
        return {
            ...
            "logz_est": {
                "display_name": "Estimated logZ from GFN samples",
                "requirements": ["model_samples"],
            },
        }
    ...
    @torch.no_grad()
    def compute_logz_metrics(self, metrics=None, sampled_states=None):
        """
        Compute an estimated logZ from samples generated by the trained GFlowNet.

        The estimate reuses the existing backward-trajectory sampling logic and the
        trajectory balance identity. Bootstrap-based uncertainty is not implemented yet.
        ...
        metrics : List[str], optional
            List of metrics to compute, by default ``None`` i.e. the evaluator's
            ``self.metrics``
        sampled_states : List, optional
            Optional terminating states to use for logZ estimation. If ``None``,
            terminating states are sampled from the current model policy.
        ...
        if sampled_states is None:
            sampled_states = []
            for b in batch_with_rest(
                0, self.config.n_logz_samples, self.config.logz_batch_size
            ):
                batch, _ = self.gfn.sample_batch(n_forward=len(b), train=False)
                sampled_states.extend(batch.get_terminating_states())
        else:
            sampled_states = list(sampled_states)
            if len(sampled_states) == 0:
                raise ValueError("sampled_states must not be empty.")
            sampled_states = sampled_states[: self.config.n_logz_samples]
        ...
        return {"metrics": {"logz_est": logz_est.item()}}
    ...
    @torch.no_grad()
    def eval(self, metrics=None, logz_states=None, **plot_kwargs):
        """
        ...
        metrics : List[str], optional
            ...
            attribute.
        logz_states : List, optional
            Optional terminating states used for ``logz_est``. If ``None``, the
            evaluator samples states from the model policy.
        ...
        if "model_samples" in reqs:
            logz_results = self.compute_logz_metrics(
                metrics=metrics, sampled_states=logz_states
            )
            all_metrics.update(logz_results.get("metrics", {}))
            all_data.update(logz_results.get("data", {}))
        ...
        if self.gfn.buffer.test is None:
            ...
            return {"metrics": all_metrics, "data": all_data}
