#!/bin/bash

#SBATCH -n 1                                # Ask for 2 CPUs
#SBATCH -c 4
#SBATCH --gpus=1
#SBATCH --mem=32G                                        # Ask for 10 GB of RAM
#SBATCH --time=56:00:00                                   # The job will run for 3 hours

module --force purge
module load python/3.10
module load cuda/11.8

source /network/projects/crystalgfn/catalyst/gflownet-dev/crystals-env3/bin/activate


HYDRA_FULL_ERROR=1 python train.py +experiments=crystals/spacegroup/fake_crystal_test_uniform_constraints_6 seed=101 gflownet.optimizer.batch_size.forward=10